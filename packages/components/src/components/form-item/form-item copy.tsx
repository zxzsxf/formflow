...
import { pickDataProps } from "../../common/props-utils";
...
export const BaseItem: React.FC<React.PropsWithChildren<IFormItemProps>> = ({
  ...
  const prefixCls = withPrefix("form-item");
  ...
  const renderLabel = () => {
    if (!label && !labelOffset) return null
    return (
      <div
        className={cls({
        ...
        style={{
          ...labelStyle,
          visibility: labelOffset ? 'hidden' : 'visible',
        }}
      >
        {renderLabelText()}
        {renderTooltipIcon()}
...
export const FormItem: ComposeFormItem = connect(
  ...
  mapProps((props, field) => {
    ...
    const takeOptionalMarkHidden = () => {
      ...
      return props.optionalMarkHidden;
    };
    ...
    return {
      ...
      asterisk: props.asterisk,
      optionalMarkHidden: takeOptionalMarkHidden(),
      extra: props.extra || field.description,
    };
  })
);
